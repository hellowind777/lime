...
export function WorkbenchCreateEntryHome({
  ...
  const promptPreview = pendingCreateConfirmation?.initialUserPrompt?.trim() || "";
  ...
  const selectedOptionRaw = confirmationFormData[CREATE_CONFIRMATION_FORM_FIELDS.option];
  const selectedOption = Array.isArray(selectedOptionRaw) ? selectedOptionRaw[0] : selectedOptionRaw;
  const noteValue = String(confirmationFormData[CREATE_CONFIRMATION_FORM_FIELDS.note] || "").trim();
      ...
      <div className="w-full max-w-[800px] relative z-10 flex flex-col items-center gap-10 animate-in fade-in slide-in-from-bottom-6 duration-700 zoom-in-95 ease-out">
        ...
        <div className="text-center space-y-6">
          <div className="flex justify-center flex-wrap items-center gap-2.5">
            {projectName && (
              <Badge variant="outline" className="border-slate-200/60 bg-white/60 dark:border-white/10 dark:bg-white/5 backdrop-blur-md text-slate-700 dark:text-slate-200 font-medium px-4 py-1.5 shadow-sm">
                <FolderOpen className="w-3.5 h-3.5 mr-1.5 text-primary" />
                {projectName}
              </Badge>
            )}
            <Badge variant="secondary" className="bg-primary/10 text-primary dark:bg-primary/20 hover:bg-primary/20 transition-colors font-medium px-4 py-1.5 backdrop-blur-md shadow-sm border border-primary/20">
            ...
            {hasPendingTask ? (
                ...
                <div className="p-8 md:p-10 md:pb-8 flex flex-col gap-8">
                  {promptPreview && (
                    <div className="relative rounded-[18px] border border-primary/20 bg-primary/5 p-6 text-sm text-slate-800 dark:text-slate-200 shadow-inner group/prompt">
                      ...
                      <div className="leading-relaxed whitespace-pre-wrap text-[15px]">{promptPreview}</div>
                    </div>
                  )}
              ...
              <div className="p-20 flex flex-col items-center justify-center text-center">
                <div className="w-24 h-24 rounded-3xl bg-gradient-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-900 border border-slate-200/50 dark:border-slate-700/50 flex items-center justify-center mb-8 shadow-inner relative group-hover:scale-105 transition-transform duration-500">
                  ...
                  <LayoutTemplate className="w-12 h-12 text-slate-400 dark:text-slate-500 relative z-10" strokeWidth={1} />
                </div>
